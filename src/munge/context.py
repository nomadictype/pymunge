from munge.error import MungeError
from munge.enums import CipherType, MACType, ZipType
import munge.native
import ctypes
import socket
import struct
...
class MungeContext(object):
    ...
    def __init__(self, ctx=None):
        ...
        """
        if ctx is not None:
            self.ctx = munge.native.munge_ctx_copy(ctx)
        else:
            self.ctx = munge.native.munge_ctx_create()
    ...
    def close(self):
        ...
        if self.ctx is not None:
            munge.native.munge_ctx_destroy(self.ctx)
            self.ctx = None
    ...
    def encode(self, payload=None):
        ...
        if isinstance(payload, bytes):
            return munge.native.munge_encode(self.ctx, payload, len(payload))
        elif payload is None:
            return munge.native.munge_encode(self.ctx, None, 0)
        else:
    ...
    def decode(self, cred):
        ...
        if isinstance(cred, bytes):
            return munge.native.munge_decode(cred, self.ctx)
        else:
    ...
    def realm(self, realm):
        ...
        self._set_option(munge.native.MUNGE_OPT_REALM,
    ...
    def ttl(self):
        ...
        * MUNGE_TTL_MAXIMUM, which specifies the maximum allowed by the
        ...
        return self._get_option(munge.native.MUNGE_OPT_TTL, ctypes.c_int)
    @ttl.setter
    def ttl(self, ttl):
        self._check_arg_type(ttl, "ttl", int)
        self._set_option(munge.native.MUNGE_OPT_TTL,
                ctypes.c_int, ttl)
    ...
    def addr4(self):
        ...
        explicitly set."""
        ip = self._get_option(munge.native.MUNGE_OPT_ADDR4, ctypes.c_ulong)
    ...
    def encode_time(self):
        ...
        return self._get_option(munge.native.MUNGE_OPT_ENCODE_TIME,
                munge.native.time_t)
    ...
    def decode_time(self):
        ...
        return self._get_option(munge.native.MUNGE_OPT_DECODE_TIME,
                munge.native.time_t)
    ...
    def socket(self):
        ...
        a str."""
        return self._get_option(munge.native.MUNGE_OPT_SOCKET,
                ctypes.c_char_p).decode('utf-8')
    @socket.setter
    def socket(self, sock):
        self._check_arg_type(sock, "socket", str)
        self._set_option(munge.native.MUNGE_OPT_SOCKET,
    ...
    def uid_restriction(self):
        ...
        uid = self._get_option(munge.native.MUNGE_OPT_UID_RESTRICTION,
                munge.native.uid_t)
        if uid == munge.native.uid_t(munge.enums.UID_ANY).value:
            return munge.enums.UID_ANY
        else:
            return uid
    @uid_restriction.setter
    def uid_restriction(self, uid_restriction):
        self._check_arg_type(uid_restriction, "uid_restriction", int)
        self._set_option(munge.native.MUNGE_OPT_UID_RESTRICTION,
                munge.native.uid_t, uid_restriction)
    ...
    def gid_restriction(self):
        ...
        gid = self._get_option(munge.native.MUNGE_OPT_GID_RESTRICTION,
                munge.native.gid_t)
        if gid == munge.native.gid_t(munge.enums.GID_ANY).value:
            return munge.enums.GID_ANY
        else:
            return gid
    @gid_restriction.setter
    def gid_restriction(self, gid_restriction):
        self._check_arg_type(gid_restriction, "gid_restriction", int)
        self._set_option(munge.native.MUNGE_OPT_GID_RESTRICTION,
                munge.native.gid_t, gid_restriction)
    ...
    def _get_option(self, option, option_type):
        self._ensure_is_open()
        val = option_type()
        result = munge.native.munge_ctx_get(self.ctx, option,
                        ctypes.byref(val))
        return val.value
    ...
    def _set_option(self, option, option_type, value):
        ...
        munge.native.munge_ctx_set(self.ctx, option, val)
